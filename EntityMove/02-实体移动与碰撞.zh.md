...
title: 实体移动过程及碰撞机制
description: Minecraft实体移动过程及碰撞机制
index: 2
is-advanced: false
---
...
在1.16.1及之前部分弹射物（如投掷物）检查的是方块的Outline Shape（即光标对准时显示的黑框）的碰撞。这会使一些碰撞错误地发生（MC-73884[^ref-21]），如撞到打开的栅栏门或草；也会使一些碰撞被错误地忽略，如36号方块的碰撞。
...
因为获取待测实体列表时会先筛选出准备从中取出实体的区段（Subchunk），且只有与以移动路径为体对角线且底面平行坐标平面的一个长方体沿各方向扩大$3 + \frac{w}{2}$（w为弹射物碰撞箱宽度）的区段会被选中，所以如果实体足够大（高度或宽度的一半大于$3 + \frac{w}{2}$），就会有一部分区域的碰撞被错误地忽略。但是在生存模式中几乎不需要考虑该问题，因为只有恶魂的碰撞箱大小达到了标准且可以与弹射物发生碰撞，而且只有在位置恰到好处，也就是高度接近坐标所在区段的上界时才会造成很少一部分碰撞的丢失。
...
1.20.1 Yarn 中 `move()` 是 `MovementType` 参数化的统一入口：
...
public void move(MovementType movementType, Vec3d movement) {
    if (this.noClip) {
        ...
        // 2. movementMultiplier 应用（蛛网/浆果丛减速）
        if (this.movementMultiplier.lengthSquared() > 1.0E-7) {
            movement = movement.multiply(this.movementMultiplier);
            this.movementMultiplier = Vec3d.ZERO;   // 用完即弃
            this.setVelocity(Vec3d.ZERO);           // 将 Motion 归零
        }
...
注意 `movementMultiplier` 的"用完即弃"设计：它在一次 `move()` 调用中被消费后立刻被置零，下一次移动时不会再被应用。这意味着蛛网和浆果丛的减速只在实体**停留其中**并**触发方块网格碰撞检查**（`slowMovement()`）后才生效一次。

### adjustMovementForCollisions：沿轴碰撞 + 步高

```java file=net/minecraft/entity/Entity.java
// adjustMovementForCollisions 的沿轴分解
...
**1.14 改动**：原文提到 1.14 前固定顺序为 Y→X→Z，1.14 后改为"大轴优先（但此处 Yarn 1.20.1 的实现是小轴优先）"。这体现了原文基于 1.16.4 的研究——1.16.4 中确实是大轴优先。1.20.1 的改动（小轴优先）可能影响了依赖精确碰撞顺序的装置（如珍珠矫正器）。

### 步高（stepHeight）的源码实现

```java file=net/minecraft/entity/Entity.java
// adjustMovementForCollisions() 中的 stepHeight 段
...
算法思路：如果直接水平移动被阻挡，尝试"向上提 stepHeight 高度 → 水平移动 → 回落到最终高度"。只有当实际水平位移大于不抬脚时的位移时，才采用抬脚路径。这正是玩家能自动走上半砖（高度 ≤ 0.6）但走不上完整方块（高度 > 0.6）的源码原因。

[^ref-21]: 见[参考文献](./08-参考文献.zh.md)第 21 条。
