...
`getBodyY(0.0625)` 在坐标 Y 上方 `standingEyeHeight * 0.0625` 处产生爆炸。TNT 的 `getEyeHeight()` 返回 `0.15F`，所以爆炸中心偏移 = `0.15 * 0.0625 = 0.009375m`——与原文引用的 `0.06125m` 不同。这个差异来自 1.16.4 到 1.20.1 之间可能的变化。1.16.4 中 TNT 的爆炸位置可能直接使用了 `posY + 0.06125`，1.20.1 中改用 `getBodyY()` 间接计算。
