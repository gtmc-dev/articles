...
初始水平 Motion 的模长为 $0.02 \times \sqrt{\sin²d + \cos²d} = 0.02$ m/gt，方向随机。Y 轴 0.2 m/gt 的向上初速度加上 -0.04 的重力加速度，确实在 `0.2 / 0.04 = 5` gt 后 Y Motion 归零——但位置最高点出现在第 4 gt（因为位移用的 Motion 在阻力前），与原文所述一致。
