...
第一次操作让空气变成了拉杆；第二次操作没有改变方块种类，只把拉杆的状态从关闭改成了开启。虽然两次变化并不相同，但它们最终都需要完成同一件事：**把一个新的方块状态写入世界。**
...
```text
确定位置与新状态
→ 写入区块（修改子区块、高度图、光照、方块实体）
→ 按 flags 通知其他系统（客户端同步、NC 更新、PP 更新链）
```
...
> [!TIP] **这些术语是什么意思？**
>
> - **调色板容器**：子区块用"状态表 + 编号"的方式压缩保存 4096 个位置的方块状态，详见[方块与方块状态](./01-方块与方块状态.zh.md)。
> - **高度图**：区块为快速查到某类最高方块而维护的缓存（如最高非透明方块、最高海洋底部方块等）。
> - **光照检查**：方块变化可能改变光照传播，需要通知光照引擎重新计算本区块及邻区光照。
> - **`onStateReplaced` / `onBlockAdded`**：旧方块状态被替换前、新方块状态生效后执行的逻辑钩子。
> - **方块实体维护**：根据新状态的需要创建、更新或移除方块实体。
> - **保存标记**：将区块标记为"已变更"，确保数据在未来写盘。
...
- 大量方块变化最终通过 `World#setBlockState` 写入世界。
- 放置前先计算初始状态并检查能否放置。
- 真正的方块状态保存在子区块中。
- 写入时会处理高度图、光照、旧新方块回调和方块实体。
- `flags` 控制客户端通知、NC 更新和 PP 更新等后续行为。
- 改变同一种方块的属性仍然是一次世界数据写入。
