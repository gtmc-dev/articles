...
- `/tick unfreeze`: Unfreezes game ticks. Some versions use `/tick freeze` to toggle freezing on and off. :hidden[Rant: I don't know why they split one command into two; it's a pain to type every time.]
...
- `/player <Name> spawn`: The first step in using fake players; the most basic use is force-loading chunks when testing machines. For example, use `/player yu_xian520 spawn` to summon a fake player named Yu Xian. :hidden[Follow Sanri Yuyin on Bilibili meow~]
...
Replay mod is familiar to streamers or anyone who has recorded MC videos :hidden[who are you calling unfamiliar!]. It records events that occur in loaded chunks during the recording period.
...
Combined with the game tick section above, we can use a certain moment of machine operation as the time origin (similar to the zero reference point in circuit analysis. :hidden[Students in electronics are no strangers to this]). Each module, and even each piston, can get a gt number label, so we can accurately analyze the sequential movement order of each part.
