...
活塞的更新顺序根据推拉动作与结构分析结果分为五种情况。下图展示了整体决策流程：

```mermaid
flowchart TD
    Start[活塞执行方块事件] --> IsRetract{是否为收回动作?}
    IsRetract -->|是| IsSticky{是粘性活塞?}
    IsRetract -->|否| ExtendCheck{移动方块列表是否为空?}

    IsSticky -->|是| CanPull{能否拉动方块?}
    IsSticky -->|否| RetractNoBlock[情况2: 活塞/粘性活塞只收回]

    CanPull -->|是| RetractWithBlock[情况4: 粘性活塞收回方块]
    CanPull -->|否| RetractFail[情况5: 粘性活塞收回失败]

    ExtendCheck -->|是| ExtendEmpty[情况1: 活塞/粘性活塞只伸出<br/>移动方块为空]
    ExtendCheck -->|否| ExtendWithBlock[情况3: 活塞/粘性活塞推动方块]
```

注：即使移动方块列表与破坏方块列表均为空，伸出动作仍然会创建活塞头b36并执行完整的更新流程。

#### 情况1：活塞/粘性活塞只伸出（移动方块为空）

当结构分析得到空的待移动列表和待破坏列表时，通常意味着活塞前方是空气。即使没有普通方块需要移动或破坏，活塞仍会执行完整的伸出流程。

![Diagram1](./img/Mermaid1.png)

**关键点**：即使不移动任何方块，活塞头b36的创建与更新仍会完整执行。这是伸出动作的必然路径。

#### 情况2：活塞/粘性活塞只收回（无方块拉动）

普通活塞收回时一定走这条路径；粘性活塞在没有进入实际拉回流程时，例如前方是空气或不满足拉回条件，也会直接移除活塞头。这里与“已经尝试拉回，但结构分析失败”的情况5不同。

![Diagram2](./img/Mermaid2.png)

**关键点**：活塞底座b36先于活塞头移除而创建；活塞头位置会在移除时发出更新。

#### 情况3：活塞/粘性活塞推动方块

当结构分析成功且移动方块列表非空时，伸出流程包含完整的方块移除、b36创建与多阶段更新：

![Diagram3](./img/Mermaid3.png)

**关键点**：
- 破坏方块先移除（`list3`逆序遍历）
- 移动方块b36按逆序创建（`list`逆序遍历，确保前方方块先到位）
- 活塞头b36在移动方块b36之后创建
- 更新分多轮：移动原位置的红石粉prepare与NC更新、破坏位置的更新、最终的活塞头与底座更新

#### 情况4：粘性活塞收回方块

当粘性活塞成功拉动方块时（结构分析返回非空移动方块列表），流程与推动类似但无活塞头b36创建，且**无活塞头位置的最终更新**：

![Diagram4](./img/Mermaid4.png)

**关键点**：
- 活塞底座b36最早创建并立即发出更新
- 之后与推动流程类似：移除破坏方块、创建移动b36、多阶段更新
- **关键差异**：源码中`move()`方法的活塞头位置更新（`world.updateNeighborsAlways(blockPos, Blocks.PISTON_HEAD)`，第366行）仅在`retract`参数为true时执行。粘性活塞收回调用`move(..., false)`（第219行），因此跳过此更新

#### 情况5：粘性活塞收回失败

当粘性活塞尝试收回但结构分析失败时（例如前方方块不可拉动、超过推动上限等），活塞仅创建活塞底座b36并发出最小更新：

![Diagram5](./img/Mermaid5.png)

**关键点**：这是唯一**不对活塞头位置发出任何更新**的情况。源码中收回动作的活塞头位置更新（`world.updateNeighborsAlways(blockPos, Blocks.PISTON_HEAD)`）仅在`retract`为真时执行，但收回失败时`move`方法直接返回false，根本不进入更新逻辑。

我们常说的"粘性活塞收回失败无更新"即指此情况。下面是实例演示：
