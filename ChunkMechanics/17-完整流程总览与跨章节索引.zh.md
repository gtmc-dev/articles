...
前面 16 章深入讲解了区块管理系统的各个机制——加载票、ChunkStatus、TACS、异步任务、ChunkHolder、计划刻、实体管理、保存卸载，乃至边缘的异常路径和 Savestate 现象。
...
| 阶段6（光照） | 光照队列积压 | 区块卡在LIGHT | 等待清空/减少抑制器 | 06 |
| 阶段10（实体） | OOM | PENDING卡住 | 计划刻失效→flush死锁→halt(1) | 16 |
| 阶段10（实体） | 实体数据损坏 | IOException→FRESH | 删除entities文件 | 16 |
| 保存阶段 | 保存冷却+Watchdog | Savestate状态分叉 | 自动恢复（数据可能丢失） | 15 |
...
| 光照抑制器原理 | 06 | LQ净增速, LevelPrioritizedQueue.poll |
| 计划刻为何失效 | 08,16 | tickingFutureReadyPredicate→isChunkLoaded→managedStatuses |
...
| 关服为何卡死 | 16 | flush(), dataAccess.awaitAll(), halt(1) |
| OOM会导致什么 | 16 | PENDING→计划刻失效→flush死锁→halt(1) |
| Savestate物品复制 | 15 | 保存冷却 + Watchdog强退 |
...
**光照计算**：02 章 LIGHT 阶段；05 章 ServerLightingProvider；06 章抑制器原理
...
**计划刻**：08 章调度器；16 章 isChunkLoaded 检查链；10 章实体状态依赖

**随机刻**：09 章频率计算；08 章 tickChunk() 执行点

**实体刻**：10 章实体管理器；07 章 entityTickingFuture；16 章 PENDING 卡死
...
**卸载流水线**：11 章 tryUnloadChunk；10 章实体卸载重试；16 章降温路径
...
**PENDING 卡死**：16 章状态机；08 章计划刻失效；11 章 flush 死锁

**Savestate 分叉**：15 章保存冷却；11 章 Watchdog 强退；10 章实体容器写时机
...
独立 light 线程可以并行处理多个光照任务，但代价是光照更新有延迟（可能数 gt）。这会导致玩家看到"光照闪烁"（客户端插值）或"黑影"（光照未传播完成）。光照抑制器利用的就是这个延迟（06 章）。

### 为什么要有保存冷却

**保存冷却**的目的是减少磁盘 I/O：一个区块如果 1 秒内被修改 100 次，没必要保存 100 次，只需要在 10 秒后保存最终状态。

代价是数据丢失风险：如果在冷却期间服务器崩溃/强退/断电，最近 10 秒的修改会丢失。Savestate 现象就是这种风险的具体体现（15 章）。

Mojang 认为这个权衡是值得的：正常关服时会等待冷却期结束，只有异常退出才会丢数据。服务器运维的任务是减少异常退出（Watchdog 调优、OOM 预防）。

## 本章小结

本章串联了前面 16 章的知识点，按照时间顺序展示了完整的加载流程、卸载流程和五条并行管线的交互方式。同时提供了关键机制速查表、阅读路线建议、常见问题快速定位、性能优化方向和典型场景追踪。
...
ChunkMechanics 系列到此结束。从 Ticket 创建到区块卸载，从正常流程到异常处理，从方块运算到实体管理，从客户端同步到保存流水线，这 17 章覆盖了区块管理系统的核心机制和边缘情况。希望这套知识体系能帮助你理解 Minecraft 服务端最复杂的子系统之一，并在遇到区块相关问题时快速定位根源。
