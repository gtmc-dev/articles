...
15. **保存节流与刷盘机制**：TACS 的 10 秒保存冷却、StorageIoWorker 异步写盘、自动保存/flush/关服的差异，以及强制退出为何会绕过最后保存。
16. **加载失败与降级机制**（高级专题）：IOException 降级、EmptyChunk 虚空区块、空 ProtoChunk 兜底，以及 crash report 如何记录异常 Future。
17. **完整流程总览与跨章节索引**：从 Ticket 创建到区块卸载的完整时间线、五条并行管线的交互、关键机制速查表、异常路径速查，以及分路线的阅读建议。
18. **Savestate 与区块保存抑制**（现象专题）：Watchdog Savestate 的跨区块状态分叉、保存失败与 OOM 区块保存抑制导致的目标区块重新生成。
19. **区块互换现象**（现象专题）：MC-161823 中 region 文件头部指针错位、坐标重定位日志、修复方式和预防手段。
20. **光照抑制现象**（现象专题）：START 与 PLAYER 票的时序差异、LQ 净增长速率、优先级队列与光照抑制导致的区块加载抑制。
21. **PendingChunk 现象**（现象专题）：PENDING 状态卡住后，计划刻失效、实体卸载重试与关服 flush 死锁的完整链路。

> [!TIP]
> **阅读顺序建议**：01-03（建立基础概念）→ **17（建立全局视图）** → 06-07（理解核心加载/温度机制）→ 04-05（深入异步架构）→ 08-16（按需阅读机制专题）→ 18-21（按需阅读现象专题）
